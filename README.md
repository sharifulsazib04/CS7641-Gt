...
In this project, we implemented Traffic Sign recognition from images using German traffic sign recognition benchmark dataset. Both supervised and unsupervised learning were utilized in Google Colab. We executed a vanilla neural network, single-layer CNN, and double-layer CNN in supervised learning. To verify the performance of our model, we added some noises in our test set. We obtained 93% sample accuracy and 78% accuracy for out of sample testing with vanilla fully connected neural network, whereas 95% sample accuracy and 75% accuracy out of sample testing using a single fully connected hidden layer with 512 nodes and a relu activation.  In the case of CNN, we achieved 92% in training and 90% in the testing set for double layer CNN and 87% accuracy for training and 86% accuracy for the test set for single-layer CNN. We can conclude that both of these were significantly better than the fully connected neural network and achieved high accuracy with a limited number of layers. Finally, 88% and 70% accuracy were achieved for 15% and 70% noise.

In unsupervised learning, we implemented both KMeans and GMM models. The scores on the test images were not up to the mark in both cases. Though PCA increases efficiency, the overall performance was not satisfied.  Since the higher number of clusters guided to unnecessary clustering, we used 43 clusters for the KMeans and GMM models. Instead of having the impact of camera angle, position, the brightness of images, background, etc. on clustering, we have successfully clustered the simple sign such as 'Stop,' 'Do not enter,' 'Yield,' etc. The overall performance of our methods depends on image quality, uniformity, and a variety of images in the classes of datasets. The traffic sign recognition system is susceptible to practical purposes. One misrecognition can lead to a catastrophic event. That is why this system has to be a flawless one. So, there is always a scope of improvement in this field of application of machine learning.
